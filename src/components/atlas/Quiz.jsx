...
export default function Quiz({ species, year, setYear, onExit, hidden, accent, onComplete }) {
  ...
  const playAgain = () => {
    eeSound.panelOpen();
    setPool(shuffledExtinct(species));
    setIdx(0);
    setScores([]);
    setPhase("guess");
  };
  ...
  return (
    <div className="absolute left-1/2 -translate-x-1/2 bottom-[195px] z-30 w-[540px] max-w-[92vw] glass-solid rounded-2xl p-5 pointer-events-auto caption-enter">
      <button onClick={() => { eeSound.click(); onExit(); }}
        className="absolute top-3 right-3 w-7 h-7 rounded-full hover:bg-white/10 flex items-center justify-center" aria-label="Exit quiz">
        <Icons.X size={13} />
      </button>
