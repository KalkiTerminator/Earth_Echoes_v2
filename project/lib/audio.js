// Synthesized UI sound design using Web Audio API
(function () {
  let ctx = null;
  let enabled = true;
  try {
    const saved = localStorage.getItem("ee_sound");
  ...
  function tone({ freq, duration = 0.25, type = "sine", peak = 0.08, attack = 0.005, slideTo = null }) {
    ...
    // Waveform taps
    if (window.__eeWave) window.__eeWave(duration, peak);
  }
  ...
  window.eeSound = {
    ...
    click() {
      // Major third chord: A5 (880) + C#6 (1108.73)
      tone({ freq: 880, duration: 0.22, peak: 0.05 });
      tone({ freq: 1108.73, duration: 0.22, peak: 0.045, attack: 0.008 });
    },
    panelOpen() {
      // Perfect fifth sweep A4 (440) -> E5 (659.25), sliding up an octave approx
...
(function () {
  let ctx = null, master = null, current = null, started = false, currentHabitat = null, enabled = true;
  ...
  function makeLayer(habitat) {
    ...
    function lfo(param, freq, depth, base) {
      ...
      const og = c.createGain(); og.gain.value = depth;
    ...
    if (habitat === "ocean") {
      ...
      const drip = () => { if (!enabled) return; blip(950, 480, 0.09, 0.035, "sine"); };
  ...
  window.eeAmbience = {
    ...
    setEnabled(v) {
      enabled = v;
      if (!master) return;
      master.gain.setTargetAtTime(v ? 1 : 0, getCtx().currentTime, 0.5);
    },
    stop() {
      if (current) { current.stop(); current = null; currentHabitat = null; }
    },
  };
})();
