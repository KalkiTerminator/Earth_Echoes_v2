...
const SynthOut = z.object({
  ...
  lat: z.number().nullable().describe("only from a source occurrence/coordinate; null if no source provides it — never invent"),
  lng: z.number().nullable().describe("only from a source occurrence/coordinate; null if no source provides it — never invent"),
