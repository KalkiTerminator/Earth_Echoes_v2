// Tweaks panel — theme, globe style, pin style, accent override, density
...
function TweaksPanel({ open, onClose, config, setConfig }) {
  ...
  const Choice = ({ value, current, onClick, children }) => (
    <button onClick={() => { window.eeSound?.click(); onClick(value); }}
  ...
  return (
    <div className="fixed top-20 right-6 z-[55] w-[340px] glass-solid rounded-2xl p-5 pointer-events-auto"
         style={{ animation: "slideIn .3s ease-out both" }}>
      ...
      <Row label="Accent override">
        <div className="flex flex-wrap gap-2">
          {accents.map((a) => (
            <button key={a.id}
              onClick={() => { window.eeSound?.click(); setConfig({ ...config, accent: a.id }); }}
              onMouseEnter={() => window.eeSound?.hover()}
              title={a.label}
      ...
      <div className="mono text-[10px] text-white/40 leading-relaxed">
        Changes persist in this session. All tweaks preview live on the globe.
      </div>
    </div>
  );
}
